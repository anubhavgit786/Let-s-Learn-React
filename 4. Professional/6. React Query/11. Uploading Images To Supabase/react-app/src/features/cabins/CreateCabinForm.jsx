  ...
  return (
    <Form onSubmit={handleSubmit(onSubmit, onError)}>
      ...
      <FormRow label="Discount" error={errors?.discount?.message}>
        <Input type="number" id='discount' defaultValue={0} {...register('discount', { required: "Mention the discount on cabin", validate: (discount)=> discount <= getValues().regularPrice || `${discount} Discount should be less than regular price ${getValues().regularPrice}`  })}  disabled={isCreating}/>
      </FormRow>
      ...
      <FormRow label="Cabin photo" error={errors?.image?.message}>
        <FileInput accept='image/*' id='image' {...register('image', { required: "Cabin photo is required" })}/>
      </FormRow>
